...
\begin{lemma}\label{lem:CW-structure-of-Thom-space}
    If the base space $B$ is a CW-complex, then the Thom space $\mrm{Th}(\xi)$ is 
...
\begin{lemma}\label{lem:Homology-of-Thom-space}
    ...
    \begin{equation*}
        H_{k+i}(\mrm{Th}(\xi),t_0;\mathbb{Z}) \cong H_i(B;\mathbb{Z}).
    \end{equation*}
\end{lemma}
\begin{proof}
    \begin{align*}
        H_{k+i}(\mrm{Th}(\xi),t_0;\mZ) &= 
...
In brief, for an oriented $k$-plane bundle $\xi$ over a base space $B$, we have 
\begin{equation*}
    H_i(B;\mZ)\cong H_{k+i}(\mrm{Th}(\xi),t_0;\mZ) \overset{\mathcal{C}-\text{isom}}{\cong} \pi_{k+i}(\mrm{Th}(\xi),t_0), \quad i<k-1.
\end{equation*}
For the universal bundle $\tilde{\gamma}^k_p$ over $\tilde{\mrm{Gr}}_k(\mR^{k+p})$, 
\begin{equation*}
    H_n(\tilde{\mrm{Gr}}_k(\mR^{k+p});\mZ) \overset{\mathcal{C}-\text{isom}}{\cong} 
    \pi_{k+n}(\mrm{Th}(\tilde{\gamma}^k_p),t_0) \twoheadrightarrow \Omega^{\mrm{SO}}_{n}, \quad k-2,p\geqslant n+2.
\end{equation*}