\chapter{微分流形}
    \section{微分形式的一些恒等式}
    \begin{definition}[Lie 导数的第一种定义]
        设 $M$ 为流形, $X\in\mathfrak{X}(M)$ 为光滑向量场, 
        则对任意 $\omega\in\Omega^k(M)$ 定义 Lie 导数 $L_X:\Omega^k(M)\rightarrow\Omega^k(M)$ 为
        \begin{equation*}
            (L_X\omega)(Y_1,\dots,Y_k) = X(\omega(Y_1,\dots,Y_k))
            - \sum_{i=1}^{k}\omega(Y_1,\dots,[X,Y_i],\dots,Y_k),
        \end{equation*}
        其中 $Y_1,\dots,Y_k\in\mathfrak{X}(M)$.
    \end{definition}
    % \begin{remark}
    %     该定义中 $X(\omega(Y_1,\dots,Y_k))$ 的含义是将 $\omega(Y_1,\dots,Y_k)$ 看成是 $C^\infty(M)$ 中的函数, 
    %     然后对该函数作用向量场 $X$.
    % \end{remark}
    \begin{definition}[Lie 导数的第二种定义]
        设 $M$ 为流形, $X\in\mathfrak{X}(M)$ 为光滑向量场, 
        则对任意 $\omega\in\Omega^k(M)$ 定义 Lie 导数 $L_X:\Omega^k(M)\rightarrow\Omega^k(M)$ 为
        \begin{equation*}
            (L_X\omega)_{p}(Y_1,\dots,Y_k) = \frac{\md}{\md t}\Big|_{t=0}\omega_{\phi_t(p)}((\phi_t)_*Y_1,\dots,(\phi_t)_*Y_k),
        \end{equation*}
        其中 $Y_1,\dots,Y_k\in\mathfrak{X}(M)$, $p\in M$, $\{\phi_t\}$ 为 $X$ 的流.
        或者我们也可以写成
        \begin{equation*}
            L_X\omega = \lim_{t\rightarrow 0}\frac{(\phi_t)^*\omega - \omega}{t} 
            = \frac{\md}{\md t}\Big|_{t=0}(\phi_t)^*\omega.
        \end{equation*}
    \end{definition}
    \begin{proposition}
        上面两种定义是等价的.
    ...
    \newpage
    \begin{proof}
        设 $\omega\in\Omega^k(M)$, 则对任意 $Y_1,\dots,Y_k\in\mathfrak{X}(M)$, 有
        \begin{align*}
            &(L_X\omega)_p(Y_1,\dots,Y_k) \\
            =& \frac{\md}{\md t}\Big|_{t=0}\omega_{\phi_t(p)}((\phi_t)_*Y_1,\dots,(\phi_t)_*Y_k) \\
            =& \frac{\md}{\md t}\Big|_{t=0}\omega_{\phi_t(p)}({\phi_t}_{*p}(Y_1|_p),\dots,{\phi_t}_{*p}(Y_k|_p)) \\
            \textcolor{red}{=}& \frac{\md}{\md t}\Big|_{t=0}\omega_{\phi_t(p)}(Y_1|_{\phi_t(p)},\dots,Y_k|_{\phi_t(p)}) \\
            \quad& + \sum_{i=1}^{k}\omega_p\left(Y_1|_p,\dots,\frac{\md}{\md t}\Big|_{t=0}{\phi_t}_{*}({Y_i}|_{\phi_{-t}(p)}),\dots,Y_k|_p\right) \\
            =& X_p(\omega(Y_1,\dots,Y_k)) - \sum_{i=1}^{k}\omega_p(Y_1|_p,\dots,[X,Y_i]_p,\dots,Y_k|_p).
        \end{align*}
        第三个等式利用了\textcolor{red}{Leibniz 法则}. 第四个等式的前半部分是因为
        \begin{align*}
            \frac{\md}{\md t}\Big|_{t=0}\omega_{\phi_t(p)}(Y_1|_{\phi_t(p)},\dots,Y_k|_{\phi_t(p)}) 
            =& \frac{\md}{\md t}\Big|_{t=0}(\omega(Y_1,\dots,Y_k))(\phi_t(p)) \\
            =& X_p(\omega(Y_1,\dots,Y_k)).
        \end{align*}
        后半部分是因为向量场 Lie 导数的定义
        \begin{equation*}
            (L_XY)(p) = \frac{\md}{\md t}\Big|_{t=0}(\phi_{-t})_*Y_{\phi_t(p)} = [X,Y]_p.
        \end{equation*}
        而 $-X$ 的流为 $\{\phi_{-t}\}$, 所以
        \begin{equation*}
            \frac{\md}{\md t}\Big|_{t=0}{\phi_t}_{*}({Y_i}|_{\phi_{-t}(p)}) = (L_{-X}Y_i)(p) = -[X,Y_i]_p.
        \end{equation*}
    \end{proof}
    \newpage
    \begin{proposition}
    ...
    \end{proposition}
    \begin{proof}[证法一]
        设 $\omega\in\Omega^k(M)$, 则对任意 $Z_1,\dots,Z_k\in\mathfrak{X}(M)$, 有
        \begin{align*}
            &(L_X(L_Y\omega))(Z_1,\dots,Z_k) \\
            =& X\big((L_Y\omega)(Z_1,\dots,Z_k)\big) 
            - \sum_{i=1}^{k}(L_Y\omega)(Z_1,\dots,[X,Z_i],\dots,Z_k) \\
            =& X\Big(Y(\omega(Z_1,\dots,Z_k)) - \sum_{j=1}^{k}\omega(Z_1,\dots,[Y,Z_j],\dots,Z_k)\Big) \\
            \quad& - \sum_{i=1}^{k}Y(\omega(Z_1,\dots,[X,Z_i],\dots,Z_k)) \\
            \quad& - \sum_{i\neq j}\omega(Z_1,\dots,[Y,Z_j],\dots,[X,Z_i],\dots,Z_k) \\
            \quad& + \sum_{i=1}^{k}\omega(Z_1,\dots,[Y,[X,Z_i]],\dots,Z_k) \\
            =& XY(\omega(Z_1,\dots,Z_k)) - \sum_{j=1}^{k}X(\omega(Z_1,\dots,[Y,Z_j],\dots,Z_k)) \\
            \quad& - \sum_{i=1}^{k}Y(\omega(Z_1,\dots,[X,Z_i],\dots,Z_k)) \\
            \quad& + \sum_{i\neq j}\omega(Z_1,\dots,[Y,Z_j],\dots,[X,Z_i],\dots,Z_k) \\
            \quad& + \sum_{i=1}^{k}\omega(Z_1,\dots,[Y,[X,Z_i]],\dots,Z_k).
        \end{align*}
        同理,
        \begin{align*}
            &(L_Y(L_X\omega))(Z_1,\dots,Z_k) \\
            =& YX(\omega(Z_1,\dots,Z_k)) - \sum_{j=1}^{k}Y(\omega(Z_1,\dots,[X,Z_j],\dots,Z_k)) \\
            \quad& - \sum_{i=1}^{k}X(\omega(Z_1,\dots,[Y,Z_i],\dots,Z_k)) \\
            \quad& + \sum_{i\neq j}\omega(Z_1,\dots,[X,Z_j],\dots,[Y,Z_i],\dots,Z_k) \\
            \quad& + \sum_{i=1}^{k}\omega(Z_1,\dots,[X,[Y,Z_i]],\dots,Z_k).
        \end{align*}
        因此
        \begin{align*}
            &([L_X,L_Y]\omega)(Z_1,\dots,Z_k) \\ 
            =& \Big(L_X(L_Y\omega)-L_Y(L_X\omega)\Big)(Z_1,\dots,Z_k) \\
            =& (XY - YX)(\omega(Z_1,\dots,Z_k)) \\
            \quad& + \sum_{i=1}^{k}\omega(Z_1,\dots,[Y,[X,Z_i]]-[X,[Y,Z_i]],\dots,Z_k) \\
            =& [X,Y](\omega(Z_1,\dots,Z_k)) - \sum_{i=1}^{k}\omega(Z_1,\dots,[[X,Y],Z_i],\dots,Z_k) \\
            =& (L_{[X,Y]}\omega)(Z_1,\dots,Z_k).
        \end{align*}
    \end{proof}
