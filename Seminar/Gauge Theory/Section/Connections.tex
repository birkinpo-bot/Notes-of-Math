...
\begin{proposition}
    ...
    We call it the \textbf{vertical bundle} of $E$ and denote it by $\mathcal{V}$.
\end{proposition}
\begin{proof}
...
\end{equation*}
So $j_p$ is a linear injection from $T_{\pi(p)} M$ to $T_p E$, 
whose image forms a distribution on $E$ denoted by $\mathcal{H}$, i.e., 
\begin{equation*}
    \textcolor{red}{\mathcal{H} := \big\{(p,j_p(v))\mid p\in E, v\in T_{\pi(p)} M\big\}.}
\end{equation*}
\begin{fact}
...
\begin{proof}
    ...
    \begin{gather*}
        ...
        a_*{}_{(p,q)}(\tilde{\gamma}_p'(0), \tilde{\gamma}_q'(0)) = (\tilde{\gamma}_p + \tilde{\gamma}_q)'(0) \in \mathcal{H}_{p+q}.
    \end{gather*}
\end{proof}
...
\begin{example}
    ...
    \begin{gather*}
        G(X_H,Y_H):= g(\pi_*(X_H), \pi_*(Y_H)), \quad G(X_V,Y_V) := g(\iota^{-1}(X_V), \iota^{-1}(Y_V)), \\
        \quad G(X_H,Y_V) := 0,
    \end{gather*} 
    where $X_H,Y_H\in \mathcal{H}$ and $X_V,Y_V\in \mathcal{V}$. 
...
\begin{definition}
    ...
    $v_H\in \mathcal{H}$ and $v_V\in \mathcal{V}$. We call $v_H$ and $v_V$ 
...
\begin{lemma}\label{lem:tangent-map-of-linear-ops-acts-on-vertical-bundle}
    For $\lambda\in\mathbb{R},\,(p,q)\in E\times_M E$ and $u,v\in E_{\pi(p)}$, we have  
    \begin{gather*}
...
\begin{proof}
    ...
    \begin{equation*}
        m_{\lambda}{}_*{}_p(u_H) + m_{\lambda}{}_*{}_p(u_V) = m_{\lambda}{}_*{}_p(u) 
        = (m_{\lambda}{}_*{}_p u)_H + (m_{\lambda}{}_*{}_p u)_V.
    \end{equation*}
    Since $\mathcal{H}$ is compatible with the linear structure of the vector bundle, 
    we have $m_{\lambda}{}_*{}_p(u_H)\in \mathcal{H}$. Besides, 
    Lemma \ref{lem:tangent-map-of-linear-ops-acts-on-vertical-bundle} implies 
    $m_{\lambda}{}_*{}_p(u_V)\in \mathcal{V}$, thus 
    \begin{equation*}
        m_{\lambda}{}_*{}_p(u_H) = (m_{\lambda}{}_*{}_p u)_H, \qquad m_{\lambda}{}_*{}_p(u_V) = (m_{\lambda}{}_*{}_p u)_V.
    \end{equation*}
    The proof for the addition is exactly the same.
\end{proof}

\begin{lemma}\label{lem:tangent-map-of-multiplication}
    For $\nu\in T_\lambda \mathbb{R}\cong \mathbb{R}$ and $v\in T_p E$, we have 
    \begin{equation*}
        m_*{}_{(\lambda,p)}(\nu,v) = \nu\cdot\iota_p(p) + m_\lambda{}_*{}_p v.
    \end{equation*}
\end{lemma}
\begin{proof}
    let $\gamma(t)$ be a curve in $E$ such that $\gamma(0) = p$ and $\gamma'(0) = v$, then 
    \begin{align*}
        m_*{}_{(\lambda,p)}(\nu,v) =& \frac{\md }{\md t}\Big|_{t=0} (\lambda + t\nu)\gamma(t) \\
        =& \frac{\md }{\md t}\Big|_{t=0}(\lambda p + t\nu p) + \frac{\md }{\md t}\Big|_{t=0} \lambda \gamma(t) \\
        =& \nu\cdot\iota_p(p) + m_\lambda{}_*{}_p v,
...
\begin{proof}[Proof of Theorem \ref{thm:connection-from-horizontal-distribution}]
    ...
    \begin{itemize}
        \item $\nabla_{f_1X_1 + f_2X_2} s = f_1 \nabla_{X_1} s + f_2 \nabla_{X_2} s$:
        \begin{align*}
            &\nabla_{f_1X_1 + f_2X_2} s(x) = \iota_{s(x)}^{-1}\Big(s_*{}_x(f_1X_1 + f_2X_2)_V\Big) \\
            =& \iota_{s(x)}^{-1}\Big(s_*{}_x(f_1(x)X_1{}_x)_V + s_*{}_x(f_2(x)X_2{}_x)_V\Big) \\
            =& \iota_{s(x)}^{-1}\Big(f_1(x) s_*{}_x(X_1{}_x)_V + f_2(x) s_*{}_x(X_2{}_x)_V\Big) \\
            =& f_1 \nabla_{X_1} s(x) + f_2 \nabla_{X_2} s(x).
        \end{align*}
        \item $\nabla_X (\lambda_1 s_1 + \lambda_2 s_2) = \lambda_1 \nabla_X s_1 + \lambda_2 \nabla_X s_2$:
        ...
        \begin{align*}
            ...
            =& \iota_{(\lambda s)(x)}^{-1}\Big(m_{\lambda}{}_*{}_{s(x)}\big(s_*{}_x(X_x)\big)_V\Big) 
        ...
        \begin{align*}
            ...
            =& \iota_{(s_1 + s_2)(x)}^{-1}\Big(a_*{}_{(s_1(x),s_2(x))}\big((s_{1*}{}_x(X_x))_V, (s_{2*}{}_x(X_x))_V\big)\Big) 
            \quad\text{(By Lemma \ref{lem:H-compatible-with-linear-str})}\\
