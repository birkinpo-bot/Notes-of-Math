...
\begin{proof}
    ...
    \begin{equation*}
        i: \pi^* E \hookrightarrow TE, \quad (p,p') \mapsto (p,\gamma'(0)).
    \end{equation*}
...
\begin{equation*}
    \iota_p: E_{\pi(p)} \to (\pi^*E)_p\subset T_p E, \quad p' \mapsto \gamma'(0),
\end{equation*}
where $\gamma(t) = p + t p'$ is a curve in $E$. 
Then $\iota_p$ is an isomorphism between $E_{\pi(p)}$ and $\ker \pi_*{}_p$.
...
\begin{equation*}
    TE = H \oplus \pi^* E, \qquad \tilde{\pi}_*: H \to \pi^* TM \text{ is an isomorphism}.
\end{equation*}
...
\begin{itemize}
    ...
    \end{align*} 
    where $m_\lambda: E \to E,\,m_\lambda(p) = \lambda p$ is the scalar multiplication by $\lambda$. 
    \item The addition:
    \begin{equation*}
        a: E\times_M E \to E, \quad a(p,q) = p + q,
    \end{equation*}
    where $E\times_M E = \{(p,q)\in E\times E: \pi(p) = \pi(q)\}$. For $u\in T_p E$ and $v\in T_q E$ such that $\pi(p) = \pi(q)$, 
...
\begin{definition}
    ...
    \begin{enumerate}
        \item $\tilde{\pi}_*: H \to \pi^* TM$ is an isomorphism, or equivalently, 
        there exists a splitting map $j: \pi^* TM \to TE$ such that $\tilde{\pi}_* \circ j = \mathrm{id}$.
        \[\begin{tikzcd}
            0 & {\pi^*E} & TE & {\pi^*TM} & 0
        ...
        \end{tikzcd}\]
        \item $H$ is invariant under the scalar multiplication on $E$, i.e. for any $\lambda\in \mathbb{R},\,p\in E$,
        \begin{equation*}
            m_{\lambda}{}_*{}_p H_p = H_{\lambda p}. 
        \end{equation*}
        \item $H$ is invariant under the addition on $E$, i.e. for any $(p,q)\in E\times_M E$,
        \begin{equation*}
            a_*{}_{(p,q)}(H_p,H_q) = H_{p+q}.
        \end{equation*} 
    \end{enumerate}
    The last two conditions are saying that $H$ is compatible with the linear structure of $E$.
\end{definition}
...
\begin{equation*}
    j_p: T_{\pi(p)} M \to T_p E, \quad j_p(v) := \tilde{\gamma}'(0).
\end{equation*}
By definition, 
\begin{equation*}
    \pi(\tilde{\gamma}(t)) = \gamma(t) \implies \pi_*{}_p(j_p(v)) = \pi_*{}_p(\tilde{\gamma}'(0)) = \gamma'(0) = v.
\end{equation*}
So $j_p$ is a linear injection from $T_{\pi(p)} M$ to $T_p E$, whose image forms a distribution on $E$ denoted by $H$, i.e., 
\begin{equation*}
    \textcolor{red}{H := \big\{(p,j_p(v))\mid p\in E, v\in T_{\pi(p)} M\big\}.}
\end{equation*}
\begin{fact}
...
\begin{proof}
    ...
    \begin{gather*}
        ...
        a_*{}_{(p,q)}(\tilde{\gamma}_p'(0), \tilde{\gamma}_q'(0)) = (\tilde{\gamma}_p + \tilde{\gamma}_q)'(0) \in H_{p+q}.
    \end{gather*}
\end{proof}
...
\begin{equation*}
    \textcolor{red}{(\nabla_X s)(x) := \iota_{s(x)}^{-1}(s_*{}_x(X_x)_V).}
\end{equation*}
The idea is taking the vertical component of $s_*{}_x(X_x)$.
Note that $\iota_{s(x)}: E_x \to (\pi^* E)_{s(x)}$ is the isomorphism defined above.
\begin{proposition}\label{prop:connection-from-horizontal-distribution}
    $\nabla$ defined above is a connection on $E$.
\end{proposition}
To prove this proposition, we need the following lemma.
\begin{lemma}\label{prop:H-compactible-with-linear-str}
    For any $\lambda\in\mathbb{R},\,(p,q)\in E\times_M E$ and $u\in TE_p,\,v\in TE_q$, we have 
    \begin{gather*}
        (m_{\lambda}{}_*{}_p u)_H = m_{\lambda}{}_*{}_p u_H, \qquad (m_{\lambda}{}_*{}_p u)_V = m_{\lambda}{}_*{}_p u_V, \\
        (a_*{}_{(p,q)}(u,v))_H = a_*{}_{(p,q)}(u_H,v_H), \qquad (a_*{}_{(p,q)}(u,v))_V = a_*{}_{(p,q)}(u_V,v_V).
    \end{gather*}
\end{lemma}
\begin{proof}
    It holds since $H$ is compatible with the linear structure of the vector bundle.
\end{proof}
\begin{lemma}\label{prop:tangent-map-of-linear-ops}
    For $\lambda\in\mathbb{R},\,(p,q)\in E\times_M E$ and $u,v\in E_{\pi(p)}$, recall that $\iota_p:E_{\pi(p)} \to (\pi^* E)_p$ 
    is the isomorphism from the fibre $E_{\pi(p)}$ to the vertical component of $T_p E$, then we have  
    \begin{gather*}
        m_{\lambda}{}_*{}_p \iota_p(u) = \iota_{\lambda p}(\lambda u), \\
        a_*{}_{(p,q)}(\iota_p(u),\iota_q(v)) = \iota_{p+q}(u+v).
    \end{gather*}
\end{lemma}
\begin{proof}
    By the definition of $\iota_p$, 
    \begin{gather*}
        m_{\lambda}{}_*{}_p \iota_p(u) = \frac{\md }{\md}\Big|_{t=0} m_{\lambda}(p + t u) 
        = \frac{\md }{\md}\Big|_{t=0} (\lambda p + t \lambda u) = \iota_{\lambda p}(\lambda u), \\
        a_*{}_{(p,q)}(\iota_p(u),\iota_q(v)) = \frac{\md }{\md t}\Big|_{t=0} (p + t u) + (q + t v) 
        = \iota_{p+q}(u+v).
    \end{gather*}
\end{proof}
\begin{proof}[Proof of Proposition \ref{prop:connection-from-horizontal-distribution}]
    We need to verify that $\nabla$ satisfies the properties of a connection. 
    \begin{itemize}
        \item $\nabla_{f_1X_1 + f_2X_2} s = f_1 \nabla_{X_1} s + f_2 \nabla_{X_2} s$:
        \begin{align*}
            &\nabla_{f_1X_1 + f_2X_2} s(x) = \iota_{s(x)}^{-1}(s_*{}_x(f_1X_1 + f_2X_2)_V) \\
            =& \iota_{s(x)}^{-1}(s_*{}_x(f_1(x)X_1{}_x)_V + s_*{}_x(f_2(x)X_2{}_x)_V) \\
            =& \iota_{s(x)}^{-1}(f_1(x) s_*{}_x(X_1{}_x)_V + f_2(x) s_*{}_x(X_2{}_x)_V) \\
            =& f_1 \nabla_{X_1} s(x) + f_2 \nabla_{X_2} s(x).
        \end{align*}
        \item $\nabla_X (\lambda_1 s_1 + \lambda_2 s_2) = \lambda_1 \nabla_X s_1 + \lambda_2 \nabla_X s_2$:
        ...
        \begin{align*}
            &\nabla_X (\lambda s)(x) = \iota_{(\lambda s)(x)}^{-1}\Big(\big((\lambda s)_*{}_x(X_x)\big)_V\Big) \\
            =& \iota_{(\lambda s)(x)}^{-1}\Big(\big(m_{\lambda}{}_*{}_{s(x)}s_*{}_x(X_x)\big)_V\Big) \\
            =& \iota_{(\lambda s)(x)}^{-1}\Big(m_{\lambda}{}_*{}_{s(x)}\big(s_*{}_x(X_x)\big)_V\Big) \quad\text{(By Prop \ref{prop:H-compactible-with-linear-str})}\\
            =& \iota_{(\lambda s)(x)}^{-1}\Big(\iota_{(\lambda s)(x)}(\lambda \nabla_X s(x))\Big) \quad\text{(By Prop \ref{prop:tangent-map-of-linear-ops})}\\
            =& \lambda \nabla_X s(x),
        \end{align*}
        For additivity,
        \begin{align*}
            &\nabla_X (s_1 + s_2)(x) = \iota_{(s_1 + s_2)(x)}^{-1}\Big(\big((s_1 + s_2)_*{}_x(X_x)\big)_V\Big) \\
            =& \iota_{(s_1 + s_2)(x)}^{-1}\Big(\big(a_*{}_{(s_1(x),s_2(x))}(s_{1*}{}_x(X_x), s_{2*}{}_x(X_x))\big)_V\Big) \\
            =& \iota_{(s_1 + s_2)(x)}^{-1}\Big(a_*{}_{(s_1(x),s_2(x))}\big((s_{1*}{}_x(X_x))_V, (s_{2*}{}_x(X_x))_V\big)\Big) \quad\text{(By Prop \ref{prop:H-compactible-with-linear-str})}\\
            =& \iota_{(s_1 + s_2)(x)}^{-1}\Big(\iota_{(s_1 + s_2)(x)}(\nabla_X s_1(x) + \nabla_X s_2(x))\Big) \quad\text{(By Prop \ref{prop:tangent-map-of-linear-ops})}\\
            =& \nabla_X s_1(x) + \nabla_X s_2(x).
        \end{align*}
        \item $\nabla_X (fs) = \md f(X)\cdot s + f\nabla_X s$:
        \begin{align*}
            &\nabla_X (fs)(x) = \iota_{(fs)(x)}^{-1}\Big(\big((fs)_*{}_x(X_x)\big)_V\Big) \\
            =& \iota_{(fs)(x)}^{-1}\Big(m_*{}_{(f(x),s(x))}\big(f_*{}_x(X_x), s_*{}_x(X_x)\big)_V\Big) \\ 
            =& \iota_{(fs)(x)}^{-1}\Big(\md f(X)_x\cdot \iota_{(fs)(x)}(s(x)) + m_{f(x)}{}_*{}_{s(x)}\big(s_*{}_x(X_x)\big)_V\Big) \\
